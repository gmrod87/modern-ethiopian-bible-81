...
// Remove every legacy visual layer. Release 39 is the only final visual system.
...
// The cream edition is fixed-theme, so the old theme toggle has no job and no grid slot.
h=h.replace(/\s*<button class=["']round["'] id=["']themeBtn["'][^>]*>[^<]*<\/button>/i,'');
...
h=h.replace(/\/recovery\.html\?v=\d+/g,'/recovery.html?v=39');
...
const sw='dist/sw.js';
if(fs.existsSync(sw)){
  let s=fs.readFileSync(sw,'utf8').replace(/const V=['"][^'"]+['"]/ ,"const V='the81-v39-root-clean'");
