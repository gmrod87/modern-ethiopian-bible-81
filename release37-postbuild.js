...
h=h.replace(/\/recovery\.html\?v=\d+/g,'/recovery.html?v=43');
h=h.replace(/<div class="heroActions"><a class="primary" href="\/recovery\.html\?v=43">Repair app<\/a><\/div>/i,'');
...
const sw='dist/sw.js';
if(fs.existsSync(sw)){
  let s=fs.readFileSync(sw,'utf8').replace(/const V=['"][^'"]+['"]/ ,"const V='the81-v43-ios-text-reset'");
