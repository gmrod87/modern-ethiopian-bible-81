module.exports=async function handler(req,res){
  ...
  if(req.method==='GET')return send(apiKey?200:503,{ready:!!apiKey,endpoint:'explain',mode:'one-shot-json',limitWords:300});
  ...
  const capWords=(text,max=300)=>{
    ...
    let out=words.slice(0,max).join(' ');const marks=[...out.matchAll(/[.!?](?=\s|$)/g)],last=marks.at(-1);
    if(last&&last.index>out.length*.62)out=out.slice(0,last.index+1);else out=out.replace(/[,:;\-–—]+$/,'')+'.';return out;
  };
  try{
    const body=typeof req.body==='string'?JSON.parse(req.body||'{}'):(req.body||{}),ctx=body.context||{};
    const reference=clean(body.reference||ctx.currentReference||'the current verse').slice(0,240),question=clean(body.question||`Explain ${reference} in more detail.`).slice(0,1200);
    ...
    const j=await r.json();let answer=clean(j.output_text||'');
    if(!answer){const parts=[];for(const item of (Array.isArray(j.output)?j.output:[]))for(const c of (Array.isArray(item?.content)?item.content:[]))if(typeof c?.text==='string')parts.push(c.text);answer=clean(parts.join(' '))}
    answer=capWords(answer,300);if(!answer)return send(502,{error:'Study AI returned no explanation'});
    const words=answer.split(/\s+/).filter(Boolean).length;console.log('Hobah explain success',JSON.stringify({reference,words}));return send(200,{answer,reference,words});
  }catch(e){if(e?.name==='AbortError'){console.error('Hobah explain timeout');return send(504,{error:'Study AI took too long to respond'})}console.error('Hobah explain failed',e?.message||e);return send(500,{error:'Study AI could not explain that'})}
};
