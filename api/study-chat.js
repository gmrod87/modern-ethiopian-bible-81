module.exports=async function handler(req,res){
  ...
  try{
    ...
    const source=`CURRENT REFERENCE: ${String(c.currentReference||'Study Library')}\n\nSCRIPTURE ON SCREEN:\n${String(c.scripture||'').slice(0,quick?2600:9000)}\n\nBOOK BACKGROUND:\n${String(c.bookBackground||'').slice(0,quick?1200:2800)}\n\nSECTION CONTEXT:\n${String(c.sectionContext||'').slice(0,quick?1000:2000)}\n\nRELEVANT EDITORIAL STUDY NOTES:\n${notes.map(n=>`[${n.reference} — ${n.type}] ${n.text}`).join('\n\n').slice(0,quick?2200:8000)}`;
    ...
    const quickInstruction=quick?' VOICE MODE: answer immediately in the first sentence, keep the explanation compact, and usually finish in two or three short paragraphs so the reader can return to Scripture quickly.':'';
