module.exports=async function handler(req,res){
  ...
  if(req.method==='GET'&&req.query&&req.query.health){res.statusCode=process.env.OPENAI_API_KEY?200:503;return res.end(JSON.stringify({ready:!!process.env.OPENAI_API_KEY}))}
  if(req.method!=='POST'){res.statusCode=405;return res.end(JSON.stringify({error:'Method not allowed'}))}
  if(!process.env.OPENAI_API_KEY){res.statusCode=503;return res.end(JSON.stringify({error:'Study AI is not configured'}))}
  try{
