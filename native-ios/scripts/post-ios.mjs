...
for(const [idiom,size,scale,px] of specs){
  ...
  await sharp(iconSvg).resize(px,px).flatten({background:'#F3EFE5'}).png().toFile(path.join(iconDir,filename));
...
if(existsSync(path.dirname(splashDir))){
  ...
  const logo=await sharp(iconSvg).resize(760,760).flatten({background:'#F3EFE5'}).png().toBuffer();
  const filenames=['splash-2732x2732.png','splash-2732x2732-1.png','splash-2732x2732-2.png'];
  for(const filename of filenames){
    await sharp({create:{width:2732,height:2732,channels:4,background:'#F3EFE5'}}).composite([{input:logo,gravity:'centre'}]).png().toFile(path.join(splashDir,filename));
  }
