...
const D='dist',V='96';
if(!fs.existsSync(path.join(D,'index.html')))throw new Error('Release 96 requires an existing Hobah dist build');
...
if(signedCI&&!fs.existsSync(manifest)){
  ...
  console.log('Release 96: downloading frozen audited Ancient Library corpus…');
...
if(fs.existsSync(manifest)){
  ...
  if(count<25)throw new Error(`Release 96 Ancient Library manifest is unexpectedly small: ${count} sections`);
  ...
  console.log(`Release 96: verified and bundled ${count} Ancient Library texts/sections (${chars.toLocaleString()} characters)`);
}else{
  ...
  console.warn('Release 96: Ancient Library corpus not bundled in this local preview build.');
}
...
console.log('Hobah Release 96: rebuilt Books navigation and clean Ancient Library reader applied');