...
if(signedCI&&!fs.existsSync(manifest)){
  ...
  console.log('Release 97: downloading frozen audited Ancient Library corpus…');
...
if(fs.existsSync(manifest)){
  ...
  if(count<25)throw new Error(`Release 97 Ancient Library manifest is unexpectedly small: ${count} sections`);
  ...
  console.log(`Release 97: verified and bundled ${count} audited source sections (${chars.toLocaleString()} characters)`);
}else{
  ...
  console.warn('Release 97: Ancient Library corpus not bundled in this local preview build.');
}
...
console.log('Hobah Release 97: clean work/chapter Ancient Library layer applied');