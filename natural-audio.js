(()=>{
  ...
  async function playChunk(){if(!enabled||!chunks[idx]||busy||stopped)return;busy=true;setState(`Loading ${mode()==='normal'?'reading':mode()==='context'?'reading + context':'advanced reading'} ${idx+1}/${chunks.length}…`);setPlay('…');try{const blob=await tts(chunks[idx]),a=ensureAudio();a.pause();clearUrl();a.src=URL.createObjectURL(blob);a.load();a.playbackRate=rate();a.onplay=()=>{setState(`${mode()==='normal'?'Normal':mode()==='context'?'Context Added':'Advanced'} • ${idx+1}/${chunks.length}`);setPlay('❚❚');setMeta();persist()};a.onpause=()=>{if(!busy){setPlay('▶');setMeta();persist()}};a.ontimeupdate=()=>{if(Math.floor(a.currentTime)%5===0)persist()};a.onended=async()=>{persist();if(stopped)return;if(idx<chunks.length-1){idx++;await playChunk()}else await finishChapter()};a.onerror=()=>{setState('Audio playback error');setPlay('▶')};await a.play()}catch(e){console.warn('Natural narration unavailable',e);setState('Natural voice unavailable');setPlay('▶')}finally{busy=false}}
