(()=>{
  let enabled=false,audio=null,chunks=[],idx=0,busy=false,books=[],sleepTimer=null,current=null,stopped=false;
  ...
  const chapterText=()=>[...document.querySelectorAll('#chapterText .verse')].map(v=>{const q=v.cloneNode(true);q.querySelectorAll('button').forEach(b=>b.remove());return clean(q.textContent)}).join(' ');
  ...
  const study=ctx=>window.MEB_STUDY_DATA?.[ctx?.slug]||null;
  const sectionFor=(a,c)=>a?.sections?.find(s=>c>=s.start&&c<=s.end)||null;
  function narrationIntro(ctx){
    if(!ctx)return'';
    const m=mode(),a=study(ctx),sec=sectionFor(a,ctx.chapter),bits=[`${ctx.title} ${ctx.chapter}.`];
    if(m==='normal')return bits.join(' ');
    ...
    if(m==='context'){
      if(a?.period)bits.push(`Historical setting: ${brief(a.period,190)}.`);
      if(sec?.note)bits.push(`This part of the book: ${brief(sec.note,220)}`);
      else if(a?.overview)bits.push(brief(a.overview,220));
      if(a?.scholarship)bits.push(`A short scholarly note: ${brief(a.scholarship,250)}`);
    }else if(m==='advanced'){
      if(a?.overview)bits.push(`Book frame: ${brief(a.overview,290)}`);
      if(a?.period)bits.push(`Historical setting: ${brief(a.period,230)}.`);
      if(sec?.note)bits.push(`Literary context: ${brief(sec.note,300)}`);
      if(a?.scholarship)bits.push(`Scholarship: ${brief(a.scholarship,340)}`);
    }
    bits.push('Now, the text.');
    return bits.join(' ');
  }
  ...
  function ensureControls(){
    ...
    if(!$('#audioModes')){
      const wrap=document.createElement('div');wrap.id='audioModes';wrap.className='audioModes';wrap.innerHTML='<span>READ MODE</span><div><button data-audio-mode="normal">Normal</button><button data-audio-mode="context">Context Added</button><button data-audio-mode="advanced">Advanced</button></div>';
      bar.insertBefore(wrap,ctr);
      wrap.querySelectorAll('[data-audio-mode]').forEach(b=>b.onclick=()=>{localStorage.setItem('meb:audioMode',b.dataset.audioMode);syncExtra();setState(`${b.textContent} mode • continues chapter to chapter`)});
    }
    $('#audioAutoNext')?.remove();
    if(!$('#audioContinuous')){const b=document.createElement('span');b.id='audioContinuous';b.className='audioContinuous';b.textContent='∞ Continuous';const close=$('#audioClose');ctr.insertBefore(b,close)}
  ...
  function syncExtra(){
    const m=mode();document.querySelectorAll('[data-audio-mode]').forEach(b=>b.classList.toggle('active',b.dataset.audioMode===m));
    const s=$('#audioSleep');if(s){const n=+(localStorage.getItem('meb:audioSleepMinutes')||0);s.classList.toggle('active',n>0);s.textContent=n?`${n}m`:'Sleep'}
  }
  ...
  async function tts(text){const r=await fetch('/api/tts',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text,voice:'marin',mode:mode()})});if(!r.ok)throw new Error(await r.text());return r.blob()}
  async function speak(text,start=0,ctx=null,includeContext=true){if(!enabled||busy)return;if(ctx)current=ctx;stopped=false;window.MEB_NATURAL_AUDIO_ACTIVE=true;const full=includeContext?narrationText(current,text):clean(text);chunks=splitText(full);if(!chunks.length)return;idx=Math.max(0,Math.min(start,chunks.length-1));showBar();$('#audioRef').textContent=current?`${current.title} ${current.chapter}`:'Read aloud';await playChunk()}
  async function playChunk(){if(!enabled||!chunks[idx]||busy||stopped)return;busy=true;setState(`Loading ${mode()==='normal'?'reading':mode()==='context'?'reading + context':'advanced reading'} ${idx+1}/${chunks.length}…`);setPlay('…');try{
    const blob=await tts(chunks[idx]),a=ensureAudio();a.pause();clearUrl();a.src=URL.createObjectURL(blob);a.load();a.playbackRate=rate();
    a.onplay=()=>{setState(`${mode()==='normal'?'Normal':mode()==='context'?'Context Added':'Advanced'} • ${idx+1}/${chunks.length}`);setPlay('❚❚');setMeta();persist()};
    a.onpause=()=>{if(!busy){setPlay('▶');setMeta();persist()}};
    a.ontimeupdate=()=>{if(Math.floor(a.currentTime)%5===0)persist()};
    a.onended=async()=>{persist();if(stopped)return;if(idx<chunks.length-1){idx++;await playChunk()}else await finishChapter()};
    a.onerror=()=>{setState('Audio playback error');setPlay('▶')};
    await a.play();
  }catch(e){console.warn('Natural narration unavailable',e);setState('Natural voice unavailable');setPlay('▶')}finally{busy=false}}
  async function finishChapter(){
    if(stopped)return;setPlay('▶');
    if(current){
      const b=books.find(x=>x.slug===current.slug),i=b?.chapters?.findIndex(x=>x.n===current.chapter)??-1;let next=b?.chapters?.[i+1],nb=b;
      if(!next){const bi=books.findIndex(x=>x.slug===current.slug);nb=books[bi+1];next=nb?.chapters?.[0]}
      if(nb&&next){
        setState(`Continuing to ${nb.title} ${next.n}…`);location.hash=route(nb.slug,next.n);
        const ok=await waitForChapter(nb.slug,next.n);if(stopped)return;
        if(ok){current={slug:nb.slug,title:nb.title,chapter:next.n};const text=chapterText();chunks=splitText(narrationText(current,text));idx=0;$('#audioRef').textContent=`${nb.title} ${next.n}`;if(chunks.length){setTimeout(()=>playChunk(),120);return}}
      }
    }
    window.MEB_NATURAL_AUDIO_ACTIVE=false;setState('Finished');persist()
  }
  ...
  function pause(){const a=ensureAudio();if(!a.paused)a.pause();setPlay('▶');setState(current?`Paused • ${current.title} ${current.chapter}`:'Paused');persist()}
  function toggle(){const a=ensureAudio();if(a.src&&!a.ended){a.paused?a.play():pause();return}const ctx=hashCtx(),t=chapterText();if(t)speak(t,0,ctx,true)}
  function jump(d){if(!chunks.length)return;idx=Math.max(0,Math.min(chunks.length-1,idx+d));busy=false;playChunk()}
  function stop(hide=true){stopped=true;window.MEB_NATURAL_AUDIO_ACTIVE=false;if(audio){audio.pause();audio.currentTime=0;clearUrl();audio.removeAttribute('src');audio.load()}chunks=[];idx=0;busy=false;setState('Ready');setPlay('▶');if(hide)$('#audioBar')?.classList.add('hidden');if('mediaSession'in navigator)try{navigator.mediaSession.playbackState='none'}catch{}}
  function startChapter(){window.speechSynthesis?.cancel();const t=chapterText(),ctx=hashCtx();if(t&&ctx){if(audio&&!audio.paused)audio.pause();busy=false;speak(t,0,ctx,true)}}
  ...
  document.addEventListener('click',e=>{if(!enabled)return;const id=e.target?.id;
    ...
    if(id==='listenVerse'){e.preventDefault();e.stopImmediatePropagation();window.speechSynthesis?.cancel();startVerse();return}
